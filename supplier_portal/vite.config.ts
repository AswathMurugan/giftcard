...
export default defineConfig({
  ...
  server: {
    // Relay runs alongside Forge. 5173 and 5174 were already taken on this
    // machine, so Relay sits on 5175 — fixed rather than left to Vite's
    // auto-increment, so a bookmarked tab keeps working and the two apps
    // never swap ports between restarts.
    port: 5175,
    strictPort: true,
