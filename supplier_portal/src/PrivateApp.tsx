...
import { SupplierSessionProvider } from '@/pages/_shared/supplier-session';
...
const ToQuotePage = lazyWithPreload(() =>
  import('@/pages/to-quote/ToQuotePage').then((module) => ({
    default: module.ToQuotePage,
  })),
);

const OrdersPage = lazyWithPreload(() =>
  import('@/pages/orders-po/OrdersPage').then((module) => ({
...
const PoDetailPage = lazyWithPreload(() =>
  import('@/pages/orders-po/PoDetailPage').then((module) => ({
    default: module.PoDetailPage,
  })),
);

const SupplierProofsPage = lazyWithPreload(() =>
  import('@/pages/proofs/SupplierProofsPage').then((module) => ({
    default: module.SupplierProofsPage,
  })),
);

const ShipmentsPage = lazyWithPreload(() =>
  import('@/pages/shipments/ShipmentsPage').then((module) => ({
    default: module.ShipmentsPage,
  })),
);

const InvoicesPage = lazyWithPreload(() =>
  import('@/pages/invoices/InvoicesPage').then((module) => ({
    default: module.InvoicesPage,
  })),
);

const MyReportsPage = lazyWithPreload(() =>
  import('@/pages/reports/MyReportsPage').then((module) => ({
...
const ROUTES: PrivateRouteDeclaration[] = [
  ...
  /**
   * The RFE pricing surface. Hidden from the rail because it is reached two
   * ways — from To Quote, and from the link emailed when the RFE goes out.
   * Same screen either way, so a supplier who never logs in still lands
  ...
  PrivateRoute({
    path: '/proofs',
    label: 'Proofs',
    icon: 'icon_-Tb_photo_check',
    element: <SupplierProofsPage />,
  }),
  PrivateRoute({
    path: '/shipments',
    label: 'Shipments',
    icon: 'icon_-Tb_truck_delivery',
    element: <ShipmentsPage />,
  }),
  PrivateRoute({
    path: '/invoices',
    label: 'Invoices',
    icon: 'icon_-Tb_file_invoice',
    element: <InvoicesPage />,
  }),
  PrivateRoute({
    path: '/reports',
    label: 'My Reports',
    icon: 'icon_-Tb_chart_histogram',
    element: <MyReportsPage />,
  }),
  /**
   * One purchase order in full. Hidden from the rail — it is always reached
   * from a specific PO on Orders, never browsed to on its own.
   */
  PrivateRoute({
    path: '/orders/:poId',
...
export function PrivateApp() {
  ...
  return (
    <AllRouteSlugsContext.Provider value={allRouteSlugs}>
      ...
      {/* Outside <Routes> so the chosen supplier survives navigation — every
          screen reads it, and no screen resolves its own. */}
