...
/**
 * Copy for a failed ADVANCE.
 *
 * `handleAdvance`'s catch branch used to print the raw signal error — an
 * operator on GC-1016 was shown "ERROR_SIGNAL_NO_ACTIVE_WORKFLOW: No active
 * workflow is waiting for signal '1bfa664f…' (workflowid 'Workflow_1287371492')".
 * Every word of that is true and none of it tells them what to do.
 *
 * The distinction is the same one `isWorkflowNotTracking` already draws for
 * Relay: a run that is gone will never accept a signal, so "try again" is a
 * lie, and the order needs a person rather than another click. Anything else
 * really is transient.
 */
export function describeAdvanceFailure(error: unknown): string {
  if (isWorkflowNotTracking(error)) {
    return 'This order is no longer being tracked by the workflow, so it cannot be advanced from here — its run has finished or expired. The stage data is unchanged; someone will need to move it on manually.';
  }
  return `Could not advance the order (${signalErrorText(error)}). Nothing has changed — try again in a moment.`;
}
