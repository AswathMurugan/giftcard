...
import { ClientSessionProvider } from '@/pages/_shared/client-session';
...
const MyOrdersPage = lazyWithPreload(() =>
  import('@/pages/my-orders/MyOrdersPage').then((m) => ({ default: m.MyOrdersPage })),
);

const ProofsPage = lazyWithPreload(() =>
  import('@/pages/proofs/ProofsPage').then((m) => ({ default: m.ProofsPage })),
);
...
const OrderDetailPage = lazyWithPreload(() =>
  import('@/pages/my-orders/OrderDetailPage').then((module) => ({
...
const ROUTES: PrivateRouteDeclaration[] = [
  ...
  PrivateRoute({
    path: '/approvals',
    label: 'Approvals',
    icon: 'icon_-Tb_writing_sign',
    element: <ApprovalsPage />,
  }),
  /**
   * One order in full. Hidden from the rail — always reached from a specific
   * order on My Orders or from a document on Approvals.
   */
  PrivateRoute({
...
export function PrivateApp() {
  ...
  return (
    <AllRouteSlugsContext.Provider value={allRouteSlugs}>
      ...
      {/* Outside <Routes> so the chosen client survives navigation. */}
