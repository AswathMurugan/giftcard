  ...
  return (
    <div className="flex flex-col gap-4" data-testid="fulfilment-panel">
      ...
      >
            ...
            {orders.map((r) => (
              ...
              >
                ...
                <span className="rounded-full bg-teal-50 px-2 py-0.5 text-[10px] font-bold uppercase text-teal-700">
                  {/* `orders.status` is a JSON column shaped {state}, so the
                      state has to be read out — rendering the object itself
                      throws "Objects are not valid as a React child". */}
                  {r.child_order?.status?.state ?? 'Open'}
                </span>
              </div>
            ))}
          </div>
        ...
        {stage === 'Award' ? (
          <div className="flex flex-wrap items-center gap-3 border-t border-border pt-3">
            <Button
              ...
              aria-busy={busy}
              disabled={busy || unordered.length === 0}
              title={
                unordered.length === 0
        ...
        {stage === 'Award' && unordered.length === 0 ? (
          <ProceedRow
