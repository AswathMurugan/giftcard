 ...
 * the two that were seeded by hand.
 *
 ...
 */
import { COMPONENT_ROLES, pct, type ComponentRole } from '@/pages/_shared/pricing';
...
export interface ClientView {
  ...
  /**
   * component role → the `pricing_template_role` row id behind that margin.
   *
   * Carried because the two writes are different queries, not an upsert:
   * `pricing_template_role_update` needs the row id, and only its absence tells
   * `setRoleMargin` to insert instead. Kept beside `margins` rather than folded
   * into it so nothing that already reads a bare margin has to change.
   */
  roleIds: Partial<Record<ComponentRole, string>>;
...
export function buildClientViews(result: ClientAdminResult | null | undefined): ClientView[] {
  ...
  return (result?.clients ?? [])
    ...
    .map((c) => {
      ...
      const roleIds: Partial<Record<ComponentRole, string>> = {};
      if (template?.id) {
...
export function activationBlockers(view: ClientView): string[] {
  const out: string[] = [];
  if (!view.templateId) {
    // The card itself already offers "Add rate card" right where this is read,
    // so this says what is missing, not where to go and do something about it.
    out.push('No rate card yet.');
    return out;
  }
