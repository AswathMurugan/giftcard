...
export const REPORTS = [
  { id: 'pipeline', icon: 'icon_-Tb_chart_funnel', title: 'Pipeline', sub: 'Open orders by stage and awarded cost' },
...
export function orderFacts(
  ...
  return (board?.orders ?? [])
    ...
    .map((o) => {
      ...
      const state =
        asText(o.tq_instance?.current_status?.tq_state_definition?.state) ||
        asText(o.status?.state) ||
        'Pending';
